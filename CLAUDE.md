...
## Git ワークフロー
- 最初にリポジトリ内のREADME.md(ディレクトリごとに複数存在する場合もある)を全て読み、コンテキストを把握すること。
- `release/*`からfeatureブランチを切る。
  - *は数字、大きい数字ほど最新、mainにマージされていない最新のreleaseブランチを探す。
  - ユーザーからベースブランチの指定がある場合はそのブランチから切る。
- コミットメッセージは簡潔で意味のある内容に
- コミットは小さく、原子的に保つ
- **直接pushせず、必ず切ったブランチにPRを出す**
- PRを出した後は、以下の全てのコマンドをPRの中で投稿することで他のAIエージェントにPRのレビューを依頼する。
  - `/gemini review`
  - `@coderabbitai review`
  - `bugbot run`
- 上記の3台のAIエージェントのレビューに対応したら、上記のコマンドを再度PRの中で投稿することで、PRのレビューを依頼し、これを繰り返す
- 本当にnitpickingなレビューしか返ってこなくなったら修正を終了し、ユーザーに報告する。
- **PRのマージはユーザーが行うので、勝手にマージしない**